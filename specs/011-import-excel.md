...
| 5 | Typed cells | A text cell is used as written. A **number** cell arrives boxed in the binary floating point Excel stores; `Convert.ToDecimal` on the boxed value turns it into a `decimal` at 15 significant digits (Excel's own precision), and no `double` is ever declared by our code. The reader's `GetDecimal` unboxes and throws, so it cannot be used. A **date** cell becomes a `DateOnly` (the time of day is dropped). Both are then written as text **in the culture and date format the mapping declares**, so they parse back to exactly the value in the sheet under any declared culture. An error cell (`#N/A`) is empty. |
